...
class API:
    ...
    def settlement_prices(
        ...
        logger.debug(f"Settlement prices sent {response.json()}")

    def full_reset(self):
        url = f"{self._cmi_url}/api/configuration/full-reset"
        response = requests.put(url, headers=_get_headers(self._auth), verify=False)
        logger.debug(f"Full reset {response.ok}")
        
    def reset_trading(self):
        url = f"{self._cmi_url}/api/round/reset"
        response = requests.put(url, headers=_get_headers(self._auth), verify=False)
        logger.debug(f"Reset trading {response.ok}")
    ...
    def news(self, message: str):
        ...
        logger.debug(f"News sent {response.ok}  message {message}")
